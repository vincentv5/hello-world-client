...
import { Send_feedback,removeServerError } from './Store/Action'; 
...
constructor(props) {
	...
	handleSubmit=(e)=> {
		...
		this.props.Send_feedback({email,name,message}).then(()=>{
	...
	render() {
		console.log(this.state.message,this.state.characters);
		const label ={
			fontWeight:"bold",
			fontSize:"15px"
		}
		...
		const serverErr = this.state.serverErr?(<div className='col-sm-7 m-auto alert alert-success'>ooopss something went trying to send feedback <br /> please trying again !</div>): '';
		return (
			<div>
			<br />
			{errMessage}{success}{serverErr}
				...
				<h6 className='ml2' style={{color:'white'}}>Leave us a feedback of our service to you!</h6>
				  ...
				  <form onSubmit={this.handleSubmit} className='br3 shadow-2'   style={{backgroundColor:'#F8F8F8'}}>
					<fieldset id="sign_up" className="ba b--transparent ph0 mh0">
					 <div className="mt3">
						...
						className={`${err}  br3 pa2 input-reset ba bg-transparent   w-100`} 
						type="text" 
						name="name"  
						id="title"  
						value={name}/>
					 ...
					 <div className="mt3">
					  ...
					  <div className="mt3">
						<label style={label}  
						className="fw6 lh-copy f6 font" 
						htmlFor="Name">Message<span>{this.state.characters}</span></label>
						<textarea 
						onFocus={this.handleFocus} 
						onChange={this.handleTextArea} 
						rows="7" className={`${err}  
						br3 pa2 input-reset ba bg-transparent   w-100`}  
						name="message"  
						value={message} 
						maxLength='150'/>
					  </div>
					...
					<div>
					  ...
					  className="btn form-control br3" 
...
export default connect(mapStateToProps,{Send_feedback,removeServerError})(SendFeedback);